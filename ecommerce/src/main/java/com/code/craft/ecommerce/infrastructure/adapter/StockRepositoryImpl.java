...
import com.code.craft.ecommerce.infrastructure.mapper.StockMapper;
...
public class StockRepositoryImpl implements StockRepository {
    ...
    public Stock saveStock(Stock stock) {
        return stockMapper.toStock(
                stockCrudRepository.save(stockMapper.toStockEntity(stock))
        );
    }

    @Override
    public List<Stock> getStockByProduct(Product product) {
        return stockMapper.toStocks(
                stockCrudRepository.findByProductEntity(productMapper.toProductEntity(product))
        );
    }
}
